...
<6> Replace `{SB_NEXTCLOUD_FPM_VERSION}` with the actual latest `nextcloud-fpm` (Alpine) image version (can be checked at the Docker Hub).
...
<9> Replace `{SB_HTTPD_VERSION}` with the actual latest `httpd` (Alpine) image version (can be checked at the Docker Hub).
