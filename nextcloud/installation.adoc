...
<5> Replace `16.0.1-fpm-alpine` with the actual latest `nextcloud-fpm` (Alpine) image version (can be checked at the Docker Hub).
...
<8> Replace `2.4.39-alpine` with the actual latest `httpd` (Alpine) image version (can be checked at the Docker Hub).
